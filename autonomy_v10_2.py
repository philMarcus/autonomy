...
def get_espn_context(state: Dict[str, Any], league: str, date_yyyymmdd: str = "", cache_seconds: int = 60, keywords: str = "") -> str:
    """Fetch a compact, up-to-the-minute ESPN scoreboard snapshot.

    Uses ESPN's public site API (no key). Selects a relevant event based on:
      live > next upcoming > most recent finished, with optional keyword tie-break.
    """
    now = time.time()
    ...
    cache_id = f"{league_key}:{date_key}:{(keywords or '').strip().lower()}"
    ...
    data = _http_get_json(url, params={"dates": date_key})
    if not isinstance(data, dict):
        return ""

    events = data.get("events") or []
    target = _espn_pick_event([ev for ev in events if isinstance(ev, dict)], keywords=keywords)
    ...
    if TELEMETRY:
        TELEMETRY.log("external_api_call", {"provider": "espn", "url": url, "date": date_key, "cached": False})
