    ...
    for pid in list(reversed(state.get("my_post_ids", [])))[0:MY_POST_SCAN_LIMIT]:
        ...
        for c in comments:
            ...
            return {
                ...
                "comment_content": shorten(c.get("content", ""), MAX_REPLY_CANDIDATE_CHARS),
...
def execute_action(client: MoltbookClient, state: Dict[str, Any], plan: Dict[str, Any], flags: Dict[str, Any]) -> bool:
    ...
    if action == "REPLY":
        ...
        post_author = author_handle(post_obj.get("author") or post_obj.get("user") or post_obj.get("username") or post_obj.get("handle") or post_obj.get("id") or "")
        is_my_post = (post_author.lower() == (MY_USERNAME or "").strip().lower()) or (post_id in set(state.get("my_post_ids", [])))
        ...
        if (not is_my_post) and cc <= 0:
            raise ValueError("REPLY blocked: unable to determine comment_count for non-own post (conservative anti-dogpile).")
        ...
        if not is_my_post:
            if cc > MAX_THREAD_COMMENTS_FOR_OUTSIDE_ENGAGEMENT:
                raise ValueError(f"REPLY blocked (dogpile): post has {cc} comments (> {MAX_THREAD_COMMENTS_FOR_OUTSIDE_ENGAGEMENT})")
            ...
            if not depth_ok:
                raise ValueError("REPLY blocked (soft rule): only depth-1 replies allowed on others' posts")
    ...
    if action == "COMMENT":
        ...
        if not is_my_post:
            ...
            if cc <= 0:
                raise ValueError("COMMENT blocked: unable to determine comment_count for non-own post (conservative anti-dogpile).")
            if cc > MAX_THREAD_COMMENTS_FOR_OUTSIDE_ENGAGEMENT:
                raise ValueError(f"COMMENT blocked (dogpile): post has {cc} comments (> {MAX_THREAD_COMMENTS_FOR_OUTSIDE_ENGAGEMENT})")
