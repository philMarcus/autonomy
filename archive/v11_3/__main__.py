    ...
    while True:
        ...
        try:
            ...
            if plan.get("update_kernel") and args.allow_kernel_update:
                ...
                if not flags.get("read_only") and not args.dry_run:
                    ...
                    kernel = new_kernel  # Update in-memory kernel for next cycle (no disk write)
                    try:
                        print(f"{Fore.CYAN}[DRY-RUN] Kernel updated in-memory (not written to disk)")
                    except:
                        pass
                    telemetry.log("kernel_update_dryrun", {
                        "cycle": iteration,
                        "reason": reason,
                        "new_length": len(new_kernel),
                    })
