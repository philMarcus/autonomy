...
FOLLOW_ON_LIKE_DEFAULT = False             # follow authors the planner labels as "liked" (DISABLED BY DEFAULT)
...
class MoltbookClient:
    def __init__(self, api_key: str, telemetry: Optional[TelemetryLogger]=None, brain_name: str = "", read_only: bool = False, gemini_client: Optional[genai.Client] = None):
    ...
    def _classify_error(http_status: int, data: Optional[dict] = None, raw_text: str = "") -> str:
        """Coarse error taxonomy for safety + debugging.
    ...
    def _has_challenge(data: Dict[str, Any]) -> bool:
        """Detect if response contains a MoltCaptcha challenge."""
        if not isinstance(data, dict):
            return False
        # Check for challenge fields based on MoltCaptcha format
        return bool(
            data.get("challenge") or
            (data.get("challenge_type") and data.get("topic") and data.get("target_ascii_sum"))
        )

    def _solve_moltcaptcha(self, challenge_data: Dict[str, Any]) -> Optional[str]:
        """Solve a MoltCaptcha challenge using the Gemini client.

        Challenge format:
        - topic: str (e.g., "cryptography")
        - target_ascii_sum: int (e.g., 295)
        - required_words: int (e.g., 10)
        - time_limit_seconds: int (e.g., 20)
        - challenge_type: str (e.g., "haiku")

        Returns the haiku solution or None if failed.
        """
        if not self.gemini_client:
            print(f"{Fore.RED}[ERROR] Cannot solve challenge: no Gemini client provided{Style.RESET_ALL}")
            return None
        ...
        # Build prompt for Gemini to generate the haiku
...
Now write your haiku about "{topic}" with ASCII sum {target_sum} and {required_words} words:"""
        ...
        try:
            ...
            if word_count == required_words and ascii_sum == target_sum:
                ...
                print(f"{Fore.YELLOW}[CHALLENGE] ✗ Solution invalid (will fail challenge){Style.RESET_ALL}")
                # Return it anyway - Moltbook will reject but we tried
                return haiku
    ...
    def _req(self, method: str, path: str, params: Optional[dict]=None, json_body: Optional[dict]=None, challenge_answer: Optional[str]=None) -> Dict[str, Any]:
        t0 = time.time()
        method = method.upper()
        if self.read_only and method != "GET":
            ...
            if self.telemetry:
                self.telemetry.log("moltbook_api_call", {
                    ...
                    "latency_ms": 0,
                    "params": params or {},
                    "req_has_body": bool(json_body),
                    "req_body_bytes": len(json.dumps(json_body or {}, ensure_ascii=False).encode("utf-8")),
                    "resp_has_body": True,
                    "resp_body_bytes": len(json.dumps(data, ensure_ascii=False).encode("utf-8")),
                    "resp_content_type": "application/json",
                    "resp_snippet": json.dumps(data, ensure_ascii=False)[:400],
                })
        ...
        # Include challenge answer if provided
        request_body = json_body
        if challenge_answer is not None:
            if request_body is None:
                request_body = {}
            request_body = dict(request_body)  # Make a copy
            request_body["challenge_answer"] = challenge_answer

        resp = self.session.request(method, url, params=params, data=json.dumps(request_body) if request_body is not None else None, timeout=30)
        dt_ms = int((time.time() - t0) * 1000)
        ...
        if self.telemetry:
            ...
            req_body_bytes = len(json.dumps(request_body)) if request_body is not None else 0
            req_has_body = (request_body is not None)
        ...
        if isinstance(data, dict):
            ...
            if not bool(data.get("success", True)):
                ...
                if err_type in ("auth_required", "ai_verification", "suspended"):
                    # Sticky block reason for this client instance (helps suppress repeat write attempts)
...
def execute_action(client: MoltbookClient, state: Dict[str, Any], plan: Dict[str, Any], flags: Dict[str, Any]) -> bool:
    ...
    # Safety: optionally run in read-only mode (collect telemetry without writing).
    ...
    if flags.get("read_only") and action in WRITE_ACTIONS:
        ...
        if state.get("telemetry"):
            state["telemetry"].log_event({"event_type": "write_skipped", "action": action, "reason": "read_only"})
        return False
    if flags.get("write_disabled") and action in WRITE_ACTIONS:
        ...
        if state.get("telemetry"):
            state["telemetry"].log_event({"event_type": "write_skipped", "action": action, "reason": str(flags.get("write_disabled_reason") or "write_disabled")})
        return False
    ...
    if action == "POST":
        ...
        if not res.get("success"):
            ...
            if err_type in ("auth_required", "ai_verification", "suspended"):
                ...
                print(f"{Fore.YELLOW}[CAPTURE] Full response captured in telemetry for analysis.{Style.RESET_ALL}")
                if TELEMETRY:
                    # Capture COMPLETE response for challenge analysis
                    TELEMETRY.log("write_blocked", {
                        "err_type": err_type,
                        "http_status": res.get("_http_status"),
                        "error": res.get("error"),
                        "hint": res.get("hint"),
                        "full_response": res  # Capture everything for analysis
                    })
...
def main():
    ...
    ap.add_argument("--read-only", action="store_true", help="Do not perform any write actions (no post/comment/reply/upvote/follow/DM). Useful for safely collecting telemetry.")
    ...
    TELEMETRY = TelemetryLogger(brain_name=brain_name, run_id=RUN_ID, base_dir=telemetry_dir, read_only=getattr(args, "read_only", False))
    ...
    client = MoltbookClient(MOLTBOOK_API_KEY, telemetry=TELEMETRY, brain_name=brain_name, read_only=bool(getattr(args,'read_only',False)), gemini_client=brain_client)
